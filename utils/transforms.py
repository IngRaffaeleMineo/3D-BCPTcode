...
class ModLabel(Transform):
    def __init__(self, key_dest, key_1, soglia_1, key_2, soglia_2):
        super().__init__()
        self.key_dest = key_dest
        self.key_1 = key_1
        self.soglia_1 = soglia_1
        self.key_2 = key_2
        self.soglia_2 = soglia_2
    
    def __call__(self, data):
        d = copy.deepcopy(data)

        if (self.soglia_1==None or self.soglia_2==None):
            return d

        if (self.soglia_1<0 or self.soglia_1>1 or self.soglia_2<0 or self.soglia_2>1):
            raise RuntimeError("Soglia 1 o soglia 2 devono essere nel range [0,1].")
        
        if (d[self.key_1] != nan):
            if (d[self.key_1] <= self.soglia_1):
                d[self.key_dest] = 1
            else:
                d[self.key_dest] = 0
        elif (d[self.key_2] != nan):
            if (d[self.key_2] <= self.soglia_2):
                d[self.key_dest] = 1
            else:
                d[self.key_dest] = 0
        else:
            raise RuntimeError("data[key_1] e data[key_2] sono entrambi NaN.")
